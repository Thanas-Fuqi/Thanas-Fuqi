#  ☯👋 Hi, I’m **@Thanas-Fuqi**

## I Like : `♪ 🎸 ☕︎ 📒 🔎`, And more ...

---

<details>
  <summary>🌐 Websites</summary>

- ✔️[alkanatron-11f](https://github.com/Thanas-Fuqi/alkanatron-11f) – Chemistry project, class 11 (2023-2024).
- ✔️[cointic-12f](https://github.com/Thanas-Fuqi/cointic-12f) – Economy project, class 12 (2024-2025).
- ❌[Dream_Journal](https://github.com/Thanas-Fuqi/Dream_Journal) – (**Unfinished**) Website containing dreams.
- ✔️[eco-vibes](https://github.com/Thanas-Fuqi/eco-vibes) – Upshift project submission, november 2023.
- ❌[light-saber-11f](https://github.com/Thanas-Fuqi/light-saber-11f) – Albanian(Grammar) project, class 11 (2023-2024).
- ✔️[mitosis-sim-11f](https://github.com/Thanas-Fuqi/mitosis-sim-11f) – English project, class 11 (2023-2024).
- ❌[Native-busters-uwu-](https://github.com/Thanas-Fuqi/Native-busters-uwu-) – AI hackathon project submission 2023.
- ❌[My_bio_website](https://github.com/Thanas-Fuqi/My_bio_website) – (**Unfinished**) My bio as a website project.

</details>

<details>
  <summary>📱 Android Apps</summary>

- ❌[College_Finder](https://github.com/Thanas-Fuqi/College_Finder) – CFA project, july 2022.
- ❌[Eco_Vibes](https://github.com/Thanas-Fuqi/Eco_Vibes) – Part of the Upshift project, november 2023.
- ✔️[Enigma_Machine](https://github.com/Thanas-Fuqi/Enigma_Machine) – Practice app, German Enigma Machine.
- ❌[Gae_Smex](https://github.com/Thanas-Fuqi/Gae_Smex) – Personal GaeSmex preview android application.
- ✔️[Text_Adventure](https://github.com/Thanas-Fuqi/Text_Adventure) – Practice app, character based choice game.

</details>

<details>
  <summary>🗄 Other Data</summary>

- ❌[100_days_python](https://github.com/Thanas-Fuqi/100_days_python) – Online python course.
- ✔️[Cave_Myth](https://github.com/Thanas-Fuqi/Cave_Myth) – Philosophy project (windows game), class 11 (2023-2024).
- ❌[CFA-2022-2024](https://github.com/Thanas-Fuqi/CFA-2022-2024) – IRL and Online coding course (july 2022 - july 2024).
- ❌[CODE](https://github.com/Thanas-Fuqi/CODE) – Various snippets of code in different languages.
- ❌[Everyday_Saves](https://github.com/Thanas-Fuqi/Everyday_Saves) – Everyday text base saves and notes.
- ❌[GaeSmex](https://github.com/Thanas-Fuqi/GaeSmex) – My GaeSmex data in excel format.
- ✔️[Games_py](https://github.com/Thanas-Fuqi/Games_py) – Various games made with python.
- ✔️[MSWLogo_Practice](https://github.com/Thanas-Fuqi/MSWLogo_Practice) – Various MSWLogo builds made for practice.
- ❌[School_Data](https://github.com/Thanas-Fuqi/School_Data) – All data from middle school (lower + upper) 2022-2025.
- ❌[Uni_Data](https://github.com/Thanas-Fuqi/Uni_Data) – All data from University.

</details>
